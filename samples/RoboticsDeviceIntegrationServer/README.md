# RoboticsDeviceIntegrationServer

A self-contained .NET console OPC UA server that demonstrates the **OPC 40010
...
```
dotnet run --project Applications/RoboticsDeviceIntegrationServer -- --host localhost --port 62830
```

Then drive it with the **same** generic bridge used for pumps
(`PumpDeviceIntegrationBridge`) to author a live USD override layer:

```
dotnet run --project Applications/PumpDeviceIntegrationBridge -- \
    --server opc.tcp://localhost:62830/RoboticsDeviceIntegrationServer \
